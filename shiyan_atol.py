...
LastEditTime: 2025-05-10 14:02:03
...
def get_fid(atol):
    real_images_folder = "./experiments/atol_real{}".format(atol)  # 真实图像文件夹路径
    generated_images_folder = "./experiments/atol_gen{}".format(atol)  # 生成图像文件夹路径
...
if __name__ == "__main__":
    ...
    atol_list = np.linspace(1, 10, 5) #步长
    for atol in atol_list:
        ...
        for j in range(0,id):
            ...
            model.prepare_to_drag(init_pts, np.ones((512, 512), dtype=np.uint8), 0.001, atol=atol)
            drag_step = 0 #步数
            if_drag = True
            image = image1
            ...
            if os.path.exists("./experiments/atol_real{}".format(atol)) == False:
                os.makedirs("./experiments/atol_real{}".format(atol))
            if os.path.exists("./experiments/atol_gen{}".format(atol)) == False:
                os.makedirs("./experiments/atol_gen{}".format(atol))

            image.save("./experiments/atol_real{}/{}.png".format(atol ,j + 1))
            image2.save("./experiments/atol_gen{}/{}.png".format(atol ,j + 1))
        ...
        ##对于每一个atol保存md,fid,time_all到文件
        with open("./experiments/atol.txt", "a") as f:
            f.write("atol: {0}, md: {1}, fid: {2}, time: {3}\n".format(atol, md, fid, time_all))

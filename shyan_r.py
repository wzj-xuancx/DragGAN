...
LastEditTime: 2025-05-09 22:12:17
...
def get_fid(r1, r2):
    real_images_folder = "./experiments/r_new1{}".format(r1)  # 真实图像文件夹路径
    generated_images_folder = "./experiments/r_new2{}".format(r2)  # 生成图像文件夹路径
...
if __name__ == "__main__":
    ...
    r1_list = np.linspace(start=1, stop=10, num=5, endpoint=True)
    r2_list = np.linspace(start=5, stop=15, num=5, endpoint=True)
    for r1 in r1_list:
        for r2 in r2_list:
            ...
            for j in range(0,id):
                ...
                for i in range(0, len(points1)):
                    if len(points1[i])!=2 or len(points2[i])!=2:
                        print("Wrong!!!!!")
                ...
                model.prepare_to_drag(init_pts, np.ones((512, 512), dtype=np.uint8), 0.001, r1, r2)
                drag_step = 0 #步数
                if_drag = True
                image = image1
                ...
                if os.path.exists("./experiments/r_new1{}".format(r1)) == False:
                    os.makedirs("./experiments/r_new1{}".format(r1))
                if os.path.exists("./experiments/r_new2{}".format(r2)) == False:
                    os.makedirs("./experiments/r_new2{}".format(r2))

                image.save("./experiments/r_new1{}/{}.png".format(r1, j + 1))
                image2.save("./experiments/r_new2{}/{}.png".format(r2, j + 1))
            ...
            #对于每一个(r1,r2)保存md,fid,time_all到文件
            with open("./experiments/new_r1_r2.txt", "a") as f:
                f.write("({0},{1}), md: {2}, fid: {3}, time: {4}\n".format(r1, r2, md, fid, time_all))
